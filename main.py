...
class ArchiveStep(GeneticStep):
    def iterate(
        ...
        if ARCHIVE_TEMP and generation%5==0:
            print(f"Archive Size: {len(ARCHIVE_TEMP)}")
            model_importance = lgb.LGBMClassifier(n_estimators=50, max_depth=7, learning_rate=0.03, num_leaves=10, boosting_type='gbdt', random_state=42, n_jobs=-1, verbose=-1, scale_pos_weight= (y_train_fold==0).sum() / (y_train_fold==1).sum())
            model_importance.fit(ARCHIVE_TV_DF_TEMP.to_numpy(), y_tv_np, categorical_feature=categorical_indices)
            ...
            original_cols = ARCHIVE_TV_DF_TEMP[features_to_keep]
            ...
            for train_idx, val_idx in skf.split(ARCHIVE_TV_DF_TEMP.to_numpy(), y_tv_np):
                ...
                model = lgb.LGBMClassifier(n_estimators=50, max_depth=7, learning_rate=0.03, num_leaves=10, boosting_type='gbdt', random_state=42, n_jobs=-1, verbose=-1, scale_pos_weight= (y_train_fold==0).sum() / (y_train_fold==1).sum())
...
model_aug.fit(ARCHIVE_TV_DF_TEMP, y_train_val, categorical_feature=categorical_indices) 
