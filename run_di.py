...
for fold, (train_idx, test_idx) in enumerate(test_skf.split(X_test_augmented, y_test), 1):
    ...
    model_final = lgb.LGBMClassifier(max_bins=63,device_type='gpu', n_estimators=350, max_depth=14, learning_rate=0.03, num_leaves=17, boosting_type='gbdt', random_state=42, verbose=-1, scale_pos_weight=(y_train_val==0).sum() / (y_train_val==1).sum())
